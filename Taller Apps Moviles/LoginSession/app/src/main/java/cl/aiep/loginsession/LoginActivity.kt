            ...
            auth.signInWithEmailAndPassword(email, password)
                .addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        ...
                        Toast.makeText(this, "Inicio de sesión exitoso.", Toast.LENGTH_SHORT).show()    // Imprime mensaje éxito al iniciar sesión
                        ...
                        when (val exception = task.exception) {
                            is FirebaseAuthInvalidUserException -> {
                                // Se muestra mensaje emergente si el usuario no está registrado
                                Toast.makeText(this, "El usuario no se encuentra registrado.", Toast.LENGTH_SHORT).show()
                            }
                            is FirebaseAuthInvalidCredentialsException -> {
                                // Se muestra mensaje emergente si las credenciales son incorrectas
                                Toast.makeText(this, "Asegúrese de ingresar la información correcta.", Toast.LENGTH_SHORT).show()
                            }
                            else -> {
                                // Se muestra mensaje emergente al ocurrir otros errores durante el inicio de sesión.
